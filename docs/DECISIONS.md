...
## 2. The defect that keeps happening — fifty-five instances
...
| 54 | **A fix for a WIDTH problem that silently removed the CENTRING, because one CSS rule was doing both jobs.** | `08 · The Invitation` is `display: flex` with `justify-center`, and `Enter` takes no `className` — so the section's only flex item had no width of its own and was ~992px *by accident*, sized by whichever child happened to be widest. Giving it an explicit width (it needed one: a `max-content` rail made it 986px inside a 360px viewport) left `justify-center` nothing to centre, and the heading, both paragraphs, both pills and the carousel all moved 176px left together. **The rig measured `containerW` = 986px — exactly right — and reported PASS.** A width and a position are two facts; measuring one and reporting the other as satisfied is §2's own shape. Found by the client on his own machine. §21.10 |
| 55 | **Reading "the 3D effect, like we have on the last section" as a mechanism instead of diffing that section against the ones he was unhappy with.** | Both candidate mechanisms — `SplitLines`' line rise and `FullBleed`'s parallax — are genuinely present in the chapter he pointed at, so the phrase was ambiguous. It was resolvable in one command: `check_entrances.mjs` had been printing `parallax 1/1 moved … invitation[0]` for weeks — one drifting photograph on a page of seven chapters, and the single thing that distinguished his reference. A day was spent building the wrong effect, measured honestly throughout (0/56/84px of travel, an LCP control arm, a solved 83px). **A correct measurement of the wrong quantity is still wrong.** "Like X" is a pointer at a rendered result, not a description of a mechanism — the client cannot name the mechanism, which is why he is pointing. §21.11 |
