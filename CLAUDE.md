...
>
> **This branch is named `feat/journal-and-mobile` for a third body of work the plan executed here never
> touches.** [`docs/superpowers/specs/2026-08-26-journal-page-design.md`](docs/superpowers/specs/2026-08-26-journal-page-design.md)
> is **approved by the client (26 Aug 2026) but not yet planned or built** — a new "Journal" page (entries,
> moderated comments, a standalone photo gallery). It is the third of three bodies of work agreed that day;
> the restructure and the reviews widget documented throughout this file are the other two, and are what
> actually shipped on this branch so far.
...
| deployed | no — an older demo, revertible with one command | no — **served the demo 20 Aug to 22 Sep 2026**, and is now its one-command rollback target (`docs/DEPLOY.md`) | **YES, since 22 Sep 2026** — `https://mahua-resorts.vercel.app` serves this and `demo` points here; not merged into anything |
...
| tests | 481 | 491 | **501** |
| touch targets / zoom (§23) | n/a — not this branch's work | n/a | **done, 27-28 Aug 2026**: the carousel pager fixed to a real 29×44px target (was 13.30-15.67×14.88 — already conformant via WCAG 2.5.8's spacing exception, corrected 28 Aug, §23.3 — grown for ergonomic comfort, not a standards fix); every other sub-44px control gets an invisible `pointer: coarse` hit-area extension, client's own ruling; the park map's collision pass now knows about the lodge marker, not only its own labels |
| density (home) | page 27.0/31.1 for `field-days`; `lodges` and `rooms` over the ceiling | page mean 33.6% (`feat/home-v2`, 20 Aug 2026), every chapter inside 45% | **page mean 32.9%, worst 61.4%, every chapter inside 45%** (re-measured 27 Aug 2026, `docs/reviews/2026-08-26-restructure/density.json` — corrected from a previously-quoted 33.6% that no committed run on this branch ever produced; `invitation`'s own reading swings with the widget's render state — see `docs/reviews/2026-08-26-restructure/density-variance-NOTE.md`; unaffected by the mobile/touch work, §23) |
...
| **Tests** | **501 on `feat/journal-and-mobile`**, confirmed by this task's own fresh `npm test` run — descended from the 26 Aug restructure's own 497 across the mobile/tablet/zoom plan's six tasks: 497 (baseline, Task 1 — a rig only, no product code)→497 (Task 2, `.tap` is CSS-only)→498 (Task 3, `+1`, the pager's own hit-area test)→501 (Task 4, `+3`, three tests added alongside the brief's own one for the map's lodge-collision fix, because the brief's worked example named symbols — `keptMobileLabels`, `reachOf`, `MAP_WIDTH`, `VANN_MAP_COPY` — that do not exist in this repository; treated as illustrative pseudocode and real tests written against the real exports instead)→501 unchanged (Task 3's own fix round, evidence and rig changes only)→501 unchanged (Task 5, measurement and reporting only, no product code or test files). Full accounting: `.superpowers/sdd/2026-08-27-mobile-tablet-and-zoom/progress.md`. Before that, descended from `feat/home-v2`'s 491, not monotonic on the way there: 491→499 (the widget component, Task 1)→484 (`ReviewCarousel` and its tests removed, Task 2)→488 (`ElfsightLoader.test.tsx`, 4 new, Task 2b — corrected in the 27 Aug fix wave, this row used to skip straight from 484 to 490 and silently absorb Task 2b's own tests into Task 3's count)→491 (the tiger's own test, Task 3 — 490 after its initial pass, +1 from fix round 1's density fix)→492 (`ExperienceStrip` copy-as-props, Task 4)→498 (the property spines, Task 5)→499 (`continues`, Task 6)→**493** (`ExperiencePair`'s 3 tests and `lib/sizes.test.ts`'s 5 rows retired with the shape they tested, net of the strip's own new coverage, Task 7)→497 (Tola's press chapter, Task 8)→497 unchanged (Task 8b, no product code touched, shadow-DOM fix to a rig only). A drop is not automatically a regression on this project — see the `SplitFeature` precedent below — and every one above is explained in `.superpowers/sdd/2026-08-26-restructure-and-reviews/progress.md`. **491 on `feat/home-v2`** (481 on `feat/image-sizing` — the branches genuinely differ; see the banner above). All green. It went 467 → 492 across the coverflow's eight tasks, then **down to 479 when `SplitFeature` was retired** — exactly the 13 parameterised cases its `lib/sizes.test.ts` entries generated, so a drop there is expected and not a regression — and back up through the guards each later change added. `npm test` must stay green before any commit claiming completion.
...
3. **Cream is the page, throughout.** Base `#F1E9D7`, second surface `#E9DFC8`, ink `#31402C`, dim
   ...
   **A second exception exists, dated and narrow, alongside the first: the client's own Tripadvisor reviews
   widget** (Elfsight, `components/ui/ReviewWidget.tsx` — `feat/home-v2` and `feat/journal-and-mobile` only,
   see the branch banner above). It currently renders solid near-black cards with white type directly on
   cream — on both property pages' `Written About` chapters and the home page's closing chapter,
   screenshotted at four widths, not assumed (`docs/reviews/2026-08-26-restructure/README.md` §5). This is
   **vendor-controlled styling the client chose, not a dark section built here** — the widget's theme is set
   on his own Elfsight dashboard, not in any markup this repository controls — and he is actively restyling
   it himself (values already given: card `#F1E9D7`, text `#31402C`, meta `#5A5240`, rating `#BB8F2E`, link
   `#7A5C18`; **the rating value is `PALETTE.gold` at ~2.5:1 on cream — non-negotiable #7 — fine for the
   circles themselves, a breach if Elfsight's theme editor ever paints a numeral in it**). Recorded the same
   way non-negotiable #5's own widget exception is: as a dated fact about a vendor embed, not licence for
   anything else on this site to reach for a dark background. `docs/DECISIONS.md` §22.3, §22.11.
...
8. **Every screen must carry weight.** No section may render more than **45%** empty space at 1440×900. If
   ...
   **On `feat/image-sizing` — the TWELVE-chapter page — re-measured 11 Aug 2026 on the build that carries
   ...
   `scripts/check_docs.mjs` — **on `feat/home-v2` the page mean was 33.6% (20 Aug 2026); on
   `feat/journal-and-mobile` it is 32.9% (27 Aug 2026)**, both in the banner at the top of this
...
**SUPERSEDED 27-28 Aug 2026 — "later" arrived, and mobile/tablet/zoom is now done.** The note below described
a sequencing choice made on 12 Aug, not a lowered bar, and the client reversed the sequencing himself:
*"I want the website experience and optimization for every zoom and every device to be on point and should
not spoil or disturbe the user experience at any point."* `docs/DECISIONS.md` §23 is the full account —
touch targets across every touch shape on every route clear WCAG 2.5.8's 24×24 floor with an invisible
extension toward the 44px comfort size (client's own ruling: the visible design does not change at any
width) — **54 controls still read under 24×24 at the two mouse-pointer shapes** (`zoom-150`/`zoom-200`),
where the touch-only gate correctly does nothing, and that is not a regression (`check_responsive.mjs`
assertion 2); the carousel pager, already conformant via WCAG 2.5.8's spacing exception before this task
touched it (corrected 28 Aug 2026, §23.3), gets a solved 29×44px hit area anyway, for ergonomic comfort
rather than to fix a standards failure; the park map's collision pass now accounts for the lodge marker, and
`check_responsive.mjs` asserts all of it across three routes, eight shapes,
including browser zoom and OS-level text scaling — every kind of zoom the client asked to be covered, to one
standard. **Four genuine findings were traced and left deliberately unfixed** (the room card stack clips its
own text at 150%/200% OS text scale, `PropertyContact` wraps into itself at the same scales, the map's
legend wastes ~1.5 screens of scroll at landscape-phone width, and the map's labels never scale with OS text
size at all) — `docs/PROJECT-STATE.md`'s owed list, `DECISIONS.md` §23.8. Read on for the ORIGINAL 12 Aug
note, kept for history:

**Desktop was the client's lens as of 12 Aug 2026, and that was a sequencing ruling, not a relaxation.**
...
node scripts/check_responsive.mjs                # overflow, WCAG 2.5.8 target size (24x24), overlapping hit areas, a per-route type floor (`--baseline` to compare against a prior run), the zoom policy, and OS text scaling at 150%/200% — 3 routes x 8 shapes. FINAL STATE (28 Aug 2026, DECISIONS.md §23): assertion 2 = 54, ALL at the two mouse-only zoom shapes where `.tap`'s touch-only gate correctly does nothing — zero under-24 on every touch shape, every route (was 222 at Task 1's baseline: 24 distinct elements including the pager, now fixed, and the room card stack, deliberately untouched). Assertion 3 = 21 pairs, byte-identical throughout — all the pre-approved room-card-stack recede overlap. Assertion 6's raw total (~2,400) is NOT the finding: traced, 97% is two rig artefacts (a per-word entrance mask this rig never scrolls past, and `.drift-frame`'s by-design parallax overflow), and the true genuinely-new count is 165 — see DECISIONS.md §23.6/§23.8 for what those are and why none is fixed here. `docs/reviews/2026-08-27-mobile/README.md`
...
**Three more lessons, from a mobile/tablet/zoom reconnaissance that was wrong three times before this
project measured it properly (27-28 Aug 2026, `docs/DECISIONS.md` §23.2):** (1) measure a production build,
never `next dev` — a dev-mode indicator was once read as a layout collision; (2) an element screenshot of a
`position: sticky` container is not what a visitor sees — it renders the whole scroll range into one frame,
where a visitor sees one viewport at a time; capture the viewport at a real scroll position instead; (3) one
frame of a scroll-driven effect proves nothing — what looked like clipped text was a transition state a rig
already covers across the whole scroll. All three produced false findings that were reported to the client
