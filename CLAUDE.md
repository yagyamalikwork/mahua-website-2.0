...
| **Tests** | **482 on `feat/home-v2`** (481 on `feat/image-sizing` — the branches genuinely differ; see the banner above). All green. It went 467 → 492 across the coverflow's eight tasks, then **down to 479 when `SplitFeature` was retired** — exactly the 13 parameterised cases its `lib/sizes.test.ts` entries generated, so a drop there is expected and not a regression — and back up through the guards each later change added. `npm test` must stay green before any commit claiming completion.
