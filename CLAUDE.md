...
> 0. [`docs/DECISIONS.md`](docs/DECISIONS.md) — **every client ruling, the forty-eight-instance defect
...
>    what shipped is a third construction that is neither. Read it before touching `RoomCardStack`,
>    `ROOM_STACK`, or `.room-slot`. Distilled from the per-task ledgers, which are
...
8. **Every screen must carry weight.** No section may render more than **45%** empty space at 1440×900. If
   ...
   **`vann-rooms` and `tola-rooms` (the property pages' rooms chapters) are 31.5% and 33.8% mean, 42.1%
   and 44.3% worst, since the card stack shipped 11 Aug 2026** — both down from 40.1% and 39.0% before it,
   and both inside the ceiling. **A number recorded mid-plan — 24.8% and 27.4% — is stale and must not be
   quoted as current: it was measured while five, then six, cards were rendering their photographs past
   the card's own edge, and `measure_density.mjs` scores an overflowing photograph as 100% imagery.**
   Fixing the clip gave real area back to cream and type, which is why the honest figure is higher, not
   lower, than the mid-plan one — the same lesson the forest tint taught below, in the opposite direction.
   Full working: `docs/DECISIONS.md` §17.
...
node scripts/check_card_stack.mjs                # the rooms card stack: cards pin and recede on time, no clipping, the deck is visible, no photo crops >25%, the last card never recedes, and every card's text is actually legible
