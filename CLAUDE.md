...
node scripts/check_responsive.mjs                # the first rig that looks at a phone, a tablet or a zoom: overflow, WCAG 2.5.8 target size (24x24), overlapping hit areas, a per-route type floor (`--baseline` to compare against a prior run), the zoom policy, and OS text scaling at 150%/200% — 3 routes x 8 shapes. Baseline (27-28 Aug 2026): assertion 2 fails on 24 distinct small links (all six chapters' pager numerals, on all three routes, plus the header logo and three footer/press links), assertion 3 fails on 21 pairs confined to the property pages' room-gallery photo-links inside the (already client-approved) sticky room card stack — very likely benign, see the task's own report — and assertion 6 finds ~2,400 clip/intersection findings, overwhelmingly the per-word `SplitLines`/`Enter` reveal spans, which do not scale with root text size. `docs/reviews/2026-08-27-mobile/baseline.json`
