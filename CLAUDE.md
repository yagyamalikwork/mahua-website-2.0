...
node scripts/check_coverflow.mjs                 # `04 · Days in the Field`, 13 assertions: the stage holds, one card in charge advancing 0→5 (LINEAR — no wraparound, and an end card growing an arrow back is a failure), neighbours veiled and never faded, one live pair of arrows, the crop bound, `animation-range` never `normal`, reduced motion, no-JS — plus TWO continuous sweeps, 360-1920px of width (which caught a card 24px off-centre from 768 to 996) and 600-1200px of HEIGHT (assertion 13, which caught an anchor-target term that was wrong only above 860px, so three fixed heights would have missed it)
