...
| deployed | no — the previous demo, revertible with one command | **YES, since 20 Aug** — `https://mahua-resorts.vercel.app` serves this, and `demo` points here. Republished 21 Aug with the drift, the zoom and the review carousel |
...
| **Tests** | **491 on `feat/home-v2`** (481 on `feat/image-sizing` — the branches genuinely differ; see the banner above). All green. It went 467 → 492 across the coverflow's eight tasks, then **down to 479 when `SplitFeature` was retired** — exactly the 13 parameterised cases its `lib/sizes.test.ts` entries generated, so a drop there is expected and not a regression — and back up through the guards each later change added. `npm test` must stay green before any commit claiming completion.
...
5. **The tiger arrives, performs, then dozes.** It is not a permanent fixture — permanent peripheral motion
   ...
   **One thing on this site now moves forever, and it is the client's own request: the guests' review
   carousel** (`REVIEWS.mode: "loop"`, 21 Aug 2026). *"Slow auto-scrolling review cards, and the scroll
   pauses when the viewer hovers over the carousel."* It was put to him that this is the first element here
   that never stops and why that rule exists; he asked for **both** it and a settling alternative to be
   built so he could choose on the real page, and has not yet ruled. **`REVIEWS.mode: "settle"` is the other
   one, one word away, and one of the two is meant to be deleted.** Until he rules, treat the loop as a
   named, dated exception rather than as this rule having been relaxed — nothing else may reach for it.
...
8. **Every screen must carry weight.** No section may render more than **45%** empty space at 1440×900. If
   ...
   **`field-days` is 27.0% mean / 31.1% worst since 18 Aug 2026 and PASSES** by 13.9 points, against
   43.9% / 57.9% for the three bands it replaced. Three figures from that work generalise beyond this
   chapter. **The lever was the photographs, not the CSS** — on 16 Aug the chapter could not reach 45% at
...
node scripts/check_reviews.mjs                    # the guests' review carousel: it moves, it pauses under the pointer and starts again, the loop's seam is exact, the panel carries the WHOLE review, and (assertion 11) contrast at EIGHT PHASES of the loop rather than at one — a card in a marquee is over a different part of the photograph in every frame, which is the one thing check_contrast_over_photos.mjs cannot do
