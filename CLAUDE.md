...
> 0. [`docs/DECISIONS.md`](docs/DECISIONS.md) — **every client ruling, the fifty-one-instance defect
...
>    fixed, the rooms card stack's mobile growth (measured larger than what he accepted), and — newest —
>    **the beside room cards' own density, which now measures over the image-sizing plan's ceiling and,
>    on their worst screen, over the general 45% one too, with every named lever spent and zero effect.**
>    §7–§15 are
...
| **Tests** | **461**, all green (was 453 before the image-sizing plan; Tasks 2, 4, 5, 6 and 7 each added coverage). `npm test` must stay green before any commit claiming completion. |
| **Evidence** | `docs/reviews/2026-08-13-image-sizing/` (the image-sizing plan's own close-out — `check_plates.mjs` and `check_room_gallery.mjs`'s first full runs, `check_card_stack.mjs`'s ninth assertion, density on all three routes including the open rooms-chapter finding, transfer on all three routes, a JS-budget delta traced to a sibling commit, and screenshots read by eye), `docs/reviews/2026-08-11-card-stack/` (the rooms card stack — the rig's eight assertions on both routes, before/after density, JS budget delta-0, and the measured mobile growth against the client's accepted projection), `docs/reviews/2026-08-10-site-navigation/` (the site-wide menu and footer — density/contrast/rule-in/header/JS-budget on all three routes, including the new menu-over-photograph contrast probes and the glass-wash fix in §16), `docs/reviews/2026-08-09-property-redesign/` (Plan 7's close-out — both property routes' density, contrast, rule-in, transfer, JS budget, and the home-page regression proof), `docs/reviews/2026-08-08-property-pages/` (Plan 6's own evidence, superseded), `2026-08-08-welcome/` (the welcome), `2026-08-08-films/` (the two films), `2026-08-07-lantern/` (the lantern), `2026-08-05-signature/` (Plan 5), `2026-08-05-scroll-craft/` (Plan 4), `2026-08-04-task-7/` (Plan 3). **Every number is re-derivable with one command** — the rigs live in `scripts/` and each one asserts. `npm run verify:budget` builds, serves, measures and propagates its exit code. |
...
8. **Every screen must carry weight.** No section may render more than **45%** empty space at 1440×900. If
   ...
   **`vann-rooms` and `tola-rooms` (the property pages' rooms chapters) were 31.5% and 33.8% mean, 42.1%
   ...
   39.0% before it, and both inside the ceiling. **That pair is now stale and must not be quoted as
   current.** The client's own 13 Aug ruling replaced the stacked composition with every card
   photo-beside-words, alternating sides — and measured 14 Aug against the shipped build, the two chapters
   now read **`vann-rooms` 43.9% mean / 49.4% worst, `tola-rooms` 44.3% / 50.1% worst** — over the
   image-sizing plan's own ceiling and, on the worst screen of each, over THIS ceiling too. Every lever the
   plan named (photo width share, words padding, card height cap) was pulled and measured with zero effect;
   see `docs/DECISIONS.md` §18 and `docs/reviews/2026-08-13-image-sizing/README.md` §2 for the full
   working and why this is recorded as open rather than silently fixed or silently accepted. **A number
   recorded mid-plan (during the card stack's own build) — 24.8% and 27.4% — is separately stale for a
   different reason and must not be quoted as current either: it was measured while five, then six, cards
   were rendering their photographs past the card's own edge, and `measure_density.mjs` scores an
   overflowing photograph as 100% imagery.** Fixing that clip gave real area back to cream and type, which
   is why the honest 11-13 Aug figure was higher, not lower, than the mid-plan one — the same lesson the
   forest tint taught below, in the opposite direction. Full working on the card stack's own construction:
...
node scripts/check_plates.mjs                    # the plate boards: continuous-width distortion sweep + the 85%-of-1440 floor (65% where a board is already at minimum columns)
