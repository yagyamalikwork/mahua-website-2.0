...
| tests | 481 | 491 | **501** |
| touch targets / zoom (§23) | n/a — not this branch's work | n/a | **done, 27-28 Aug 2026**: the carousel pager fixed to a real 29×44px target (was 13×15, the only WCAG 2.5.8 failure on the site); every other sub-44px control gets an invisible `pointer: coarse` hit-area extension, client's own ruling; the park map's collision pass now knows about the lodge marker, not only its own labels |
...
touch targets on every route now clear WCAG 2.5.8's 24×24 floor with an invisible extension toward the 44px
comfort size (client's own ruling: the visible design does not change at any width), the carousel pager
(the one real standards failure) has a solved 29×44px hit area, the park map's collision pass now accounts
for the lodge marker, and `check_responsive.mjs` asserts all of it across three routes, eight shapes,
