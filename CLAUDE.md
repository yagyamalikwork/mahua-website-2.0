...
8. **Every screen must carry weight.** No section may render more than **45%** empty space at 1440×900. If
   ...
   **`field-days` is 48.1% mean / 55.0% worst since the coverflow landed 17 Aug 2026**, against 43.9% /
   57.9% for the three bands it replaced. **The worst screen improved and the mean did not**, and `passesWorst`
   is the field this rule binds on. It is still over the ceiling and **cannot reach it on today's
   photographs** — the card's own width is the only lever, a 2.289:1 panorama in a 16:9 box is drawn at
   1.288× that width, and four of the six card frames are 1163px files already served at exactly 1.00. The
   ceiling is a 903px card; 45% needs ~1,090px. **`cardMaxPx: 1120` was swept and measures 36.7% / 42.3%** —
   it needs only the files (`docs/OWED-ORIGINALS.md`). Two figures from that work are worth carrying beyond
   this chapter. **`cardMaxPx` was 560 for a day on the strength of an analogy to `ROOM_STACK.heightMax` and
   was never swept** — the same defect shape as the beside room cards below, and at 560 the chapter measured
   70.7% / 83.1%. And **a band sitting on a chapter boundary scores as a JOIN**, excluded from that chapter's
   own mean and surfacing only in the page's worst: an arm that put the tiger film in its own band read a
   flattering 48.6% mean while taking the *page's* worst screen to 77.1%.
...
node scripts/check_coverflow.mjs                 # `04 · Days in the Field`: the stage holds, one card in charge advancing 0→7, neighbours veiled and never faded, one live pair of arrows, the loop, the 25% crop bound, reduced motion, no-JS — AND (assertion 7) a continuous 360-1920px sweep in 20px steps, which is what caught the card sitting 24px off-centre from 768 to 996px (17 Aug 2026)
